...
This is a very simple object-oriented programming language compiler which is written in C. There is no expectation that this will every be used by anyone. It's just a fun hobby project.
...
* Curly braces to delimit blocks. Semicolons are used to introduce single-line comments.
* Simplified type system, but is strongly typed with casting.
* Function overloads by parameter type. Functions have a syntax of ``name(parameters)(returns)``. Parameters are passed to the function by value and returns are passed as references. Returns not assigned in the function is a warning. Returns accessed by value in a function is a syntax error, and duplicate names are a syntax error. This means that a function does not "return" anything. A syntax such as ``name.name.func()().name`` makes no sense.
* Nothing type takes the place of the ``void`` type in C. This is used to allow an object to have a type assigned at run time.
* Classes with single inheritance only. Class definitions can contain no assignments or function definitions. The parent attributes and methods are accessed by name with the '.' operator.
...
* Exceptions using the non-local goto feature of ANSI C.
